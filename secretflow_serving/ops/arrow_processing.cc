...
ArrowProcessing::ArrowProcessing(OpKernelOptions opts)
  ...
  if (trace_content.empty()) {
    dummy_flag_ = true;
    SPDLOG_INFO("arrow_processing: trace_content is empty,arrow_processing not work");
    return;
  }
  ...
  if (compute_trace_.func_traces().empty()) {
    dummy_flag_ = true;
    SPDLOG_INFO("arrow_processing: compute_trace_.func_traces is empty,arrow_processing not work");
    return;
  }
...
void ArrowProcessing::DoCompute(ComputeContext* ctx) {
 ...
 // 日志埋点，打印输入的数据表信息
  SPDLOG_INFO("arrow_processing input: {}", ctx->inputs.front()[0]->ToString());
  ...
  // 日志埋点，打印输入的数据表信息
  SPDLOG_INFO("arrow_processing output: {}", ctx->output->ToString());
}
